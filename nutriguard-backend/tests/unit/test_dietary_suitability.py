...
def test_english_incompatibility_keywords_are_still_reported_as_false():
    """Positive incompatibility evidence is preserved (not lost by the fix)."""
    product, _ = fallback_local_analysis("X", "wheat flour, whey powder, pork gelatin, salt", [])
...
def test_mixed_language_text_reports_only_the_supported_incompatibilities():
    ...
    assert product.is_lactose_free is False  # English "milk" hit
    assert product.is_vegan is False
...
def _ing(**flags):
    base = dict(is_gluten=None, is_lactose=None, is_vegan=None, is_vegetarian=None, is_halal=None, is_kosher=None)
    base.update(flags)
    return SimpleNamespace(**base)


def test_catalog_ingredient_flags_are_evidence_in_any_language():
    resolved = ds.resolve_flags(None, "Пшенично брашно", [_ing(is_gluten=True, is_vegetarian=False)])
...
def test_supported_incompatibility_still_warns_and_suitability_never_does():
    product, ingredients = fallback_local_analysis("X", "wheat flour, milk, pork", [])
    product.sugar_grams = product.sodium_mg = product.saturated_fat_grams = 0.0
